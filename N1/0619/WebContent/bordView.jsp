...
<div id="bordView">
	...
	<table id="tbordView">
		<form action="#" method="POST" name="bordViewForm">
			...
			<tr>
				<td>글제목:</td>
				<td><input type="text" name="title" value="<%=list.getTitle()%>">
				</td>
			</tr>
			<tr>
				<td>글내용:</td>
			...
			<tr>
				<td colspan="2">
					...
					<span onclick="updateGo()">글수정</span>
...
<script>
	function updateGo(){
		...
		var bordViewForm=document.bordViewForm;	// form 전체 name	
		var no=bordViewForm.no;    // 전체form 의 name="no"
		var title=bordViewForm.title;  // name="title" 
		var content=bordViewForm.content;	//name="content"	
		
		alert("글수정 페이지로 이동");	
		
		location.href="bordUpdate.jsp?no="+no.value+
					   "&title="+title.value+
