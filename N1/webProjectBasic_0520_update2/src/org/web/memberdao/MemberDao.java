...
public class MemberDao {
	...
	public ArrayList<MemberDTO> members() {
		...
		try {
			...
			while (rs.next()) {
				/*
				 * String userID = rs.getString(1); String userPW = rs.getString(2); String
				 * userNAME = rs.getString(3); int userAGE = rs.getInt(4);
				 * 
				 * MemberDTO dto = new MemberDTO(userID, userPW, userNAME, userAGE);
				 * lists.add(dto);
				 */

				lists.add(new MemberDTO(rs.getString(1), rs.getString(2), rs.getString(3), rs.getInt(4)));// 위의 과정을 한번에
																											// 처리
			}
